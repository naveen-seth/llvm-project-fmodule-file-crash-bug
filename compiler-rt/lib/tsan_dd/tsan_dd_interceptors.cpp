...
//===----------------------------------------------------------------------===//
//
// This file is a part of ThreadSanitizer-DD, a deadlock detector.
//===----------------------------------------------------------------------===//
...
#include "tsan_dd_rtl.h"

using namespace __tsan_dd;
...
INTERCEPTOR(int, pthread_mutex_init, pthread_mutex_t *m,
  ...
  if (res == 0) {
    ...
    MutexInit(cur_thread_dd(), (uptr)m, reentrant, pc);
  }
  return res;
}
...
INTERCEPTOR(int, pthread_mutex_destroy, pthread_mutex_t *m) {
  ...
  if (res == 0 || res == errno_EBUSY)
    MutexDestroy(cur_thread_dd(), (uptr)m, pc);
  return res;
}
...
INTERCEPTOR(int, pthread_mutex_lock, pthread_mutex_t *m) {
  uptr pc = GET_CURRENT_PC();
  InitThread();
  MutexBeforeLock(cur_thread_dd(), (uptr)m, true, pc);
  int res = REAL(pthread_mutex_lock)(m);
  MutexAfterLock(cur_thread_dd(), (uptr)m, true, false, pc);
  return res;
}
...
INTERCEPTOR(int, pthread_mutex_trylock, pthread_mutex_t *m) {
  ...
  if (res == 0)
    MutexAfterLock(cur_thread_dd(), (uptr)m, true, true, pc);
  return res;
}
...
INTERCEPTOR(int, pthread_mutex_unlock, pthread_mutex_t *m) {
  uptr pc = GET_CURRENT_PC();
  InitThread();
  MutexBeforeUnlock(cur_thread_dd(), (uptr)m, true, pc);
...
INTERCEPTOR(int, pthread_spin_init, pthread_spinlock_t *m, int pshared) {
  ...
  if (res == 0)
    MutexInit(cur_thread_dd(), (uptr)m, false, pc);
  return res;
}
...
INTERCEPTOR(int, pthread_rwlock_rdlock, pthread_rwlock_t *m) {
  uptr pc = GET_CURRENT_PC();
  InitThread();
  MutexBeforeLock(cur_thread_dd(), (uptr)m, false, pc);
  int res = REAL(pthread_rwlock_rdlock)(m);
  MutexAfterLock(cur_thread_dd(), (uptr)m, false, false, pc);
  return res;
}
...
INTERCEPTOR(int, pthread_rwlock_tryrdlock, pthread_rwlock_t *m) {
  ...
  if (res == 0)
    MutexAfterLock(cur_thread_dd(), (uptr)m, false, true, pc);
  return res;
}
...
INTERCEPTOR(int, pthread_rwlock_unlock, pthread_rwlock_t *m) {
  uptr pc = GET_CURRENT_PC();
  InitThread();
  MutexBeforeUnlock(cur_thread_dd(), (uptr)m, false, pc);
