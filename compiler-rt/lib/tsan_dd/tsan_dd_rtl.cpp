...
Thread *cur_thread_dd() { return thr_tls.is_inited ? &thr_tls : nullptr; }
...
static void ReportDeadlock(Thread *thr, DDReport *rep) {
  ...
  Printf("==================\n");
  Printf("%sWARNING: ThreadSanitizer: lock-order-inversion (potential "
         "deadlock) (pid=%d)%s\n",
         d.Warning(), (int)internal_getpid(), d.Default());
  ...
  for (int i = 0; i < rep->n; i++) {
    ...
    if (i == 0 && !flags()->second_deadlock_stack) {
      Printf("    HINT: use TSAN_DD_OPTIONS=second_deadlock_stack=1 to get "
...
static void ReportMutexMisuse(Thread *thr, const char *what, uptr addr, uptr pc,
  ...
  Lock lock(&ctx->report_mutex);
  Printf("==================\n");
  Printf("%sWARNING: ThreadSanitizer: %s on %smutex 0x%zx%s (pid=%d)%s\n",
         d.Warning(), what, d.Mutex(), addr, d.Default(),
...
static void InitializeFlags() {
  ...
  parser.ParseStringFromEnv("TSAN_DD_OPTIONS");
  if (!f->report_bugs) {
...
void ThreadInit(Thread *thr) {
  ...
  thr->shadow_stack = static_cast<uptr *>(MmapNoReserveOrDie(
      kShadowStackSize * sizeof(uptr), "tsan_dd shadow stack"));
